...
def hyper_discussion():
    ...
    with HyperProcess(telemetry=Telemetry.DO_NOT_SEND_USAGE_DATA_TO_TABLEAU, parameters=parameters) as hyper:
        with Connection(hyper.endpoint, 'data.hyper', CreateMode.CREATE_AND_REPLACE) as connection:
            ...
            toc = timer()
    print(f"hyper (compiled) result: {hyper_result} (computated in {toc - tic - time_explain_query}s) - planning time: {time_explain_query}")
    ...
    with HyperProcess(telemetry=Telemetry.DO_NOT_SEND_USAGE_DATA_TO_TABLEAU, parameters=parameters) as hyper:
        with Connection(hyper.endpoint, 'data.hyper', CreateMode.CREATE_AND_REPLACE) as connection:
            ...
            hyper_result = connection.execute_list_query(query)
            toc = timer()
    print(f"hyper (interpreted) result: {hyper_result} (computated in {toc - tic - time_explain_query}s) - planning time: {time_explain_query}s")
